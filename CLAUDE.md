...
- `/weekly-log` — Stage 4 workflow: runs `weekly-activity` → `workday-timelogger` → `teams-messenger` through the harness in `workflow/run_weekly_log.py`, with automated guardrails between steps, sentinel-gated human punch-outs (enforced by `.claude/hooks/require_sentinel.py`), and a per-step token/cost audit log under `workflow/runs/<run_id>/`. Usage: `/weekly-log --test` or `/weekly-log --week 2026-09-14..2026-09-18 --hours "Mon 8, Tue 8, Wed 8, Thu 8, Fri 8"`
